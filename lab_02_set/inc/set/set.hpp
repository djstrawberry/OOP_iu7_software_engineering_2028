...
Set<T>::Set(R&& range)
{
    std::ranges::for_each(range, [this](const T& value){ this->add(std::move(value)); });
}   
...
{
    std::ranges::for_each(begin, end, [this](const T& value){ this->add(value); });
}
...
{
    std::ranges::for_each(other, [this](const C& el){ this->add(el); });
    return *this;
}
...
Set<std::common_type_t<T, U>> Set<T>::make_intersection(const Set<U>& other) const
{
    ...
    std::ranges::for_each(filtered, [&set_intersection](const auto& el){ set_intersection.add(el); });
